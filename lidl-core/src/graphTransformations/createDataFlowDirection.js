...
export default function createDataFlowDirection(graph) {
  ...
  graph
    ...
    .forEach((theEdge) => {
      ...
      try {
        theEdge.from.ports = mergeInterface(
          portOnOrigin,
          conjugateInterface(portOnDestination),
        );
      ...
      try {
        theEdge.to.ports = mergeInterface(
          portOnDestination,
          conjugateInterface(portOnOrigin),
        );
  ...
  graph
    ...
    .forEach((theNode) => {
      ...
      graph
        ...
        .forEach((theEdge) => {
          // try{
          theNode.ports[theEdge.from.index] = mergeInterface(
            theNode.ports[theEdge.from.index],
            theEdge.from.ports,
          );
        ...
        .forEach((theEdge) => {
          // try{
          theNode.ports[theEdge.to.index] = mergeInterface(
            theNode.ports[theEdge.to.index],
            theEdge.to.ports,
          );
