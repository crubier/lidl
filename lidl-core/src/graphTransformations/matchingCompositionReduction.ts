...
function findAllMatchingCompositionNodesAndTheirAffectationCondition(
  graph,
  node,
) {
  ...
  function visitTheAffectation(n, fromWhichSide) {
    ...
    let conditionForCurrentAffectation = graph
    ...
    let compos = graph
      ...
      .filter(
        (e) =>
          e.from.index === 3 - fromWhichSide &&
          e.to.node.temporarilyMarkedDuringCompositionTraversal !== true,
      )
      .map((e) => ({
    ...
    .forEach((x) => {
      ...
      if (allConds.length == 0) {
        ...
        if (allConds.length == 1) {
          // We do not have to create a new Synthetic node in this case
          x.simplifiedCond = x.condition[0][0];
        } else {
          let satProblem = _(x.condition)
            .map((y) =>
              _(y)
          ...
          let allEdgesToConds = _(allConds)
            .map((c, index) => ({
              cond: c.cond,
...
export default function matchingCompositionReduction(graph) {
  ...
  graph.reduceNodes(
    ...
    (theResult, n1) => {
      let matchingCompoNodes = simplifyPathExpression(
        graph,
        findAllMatchingCompositionNodesAndTheirAffectationCondition(graph, n1),
      );
  ...
  graph
    ...
    .forEach((n1) => {
      ...
      graph
        ...
        .reject(
          (e) =>
            (_.isUndefined(e.from.compositionElementName) &&
              _.isUndefined(e.from.coCompositionElementName)) ||
            (_.isUndefined(e.to.compositionElementName) &&
              _.isUndefined(e.to.coCompositionElementName)),
        )
        .forEach((e) => {
          // console.log('iiii '+e.id);
          let origin =
            e.from.isCoPort === true
              ? coPortNodes[e.from.coCompositionElementName]
              : portNodes[e.from.compositionElementName];
          let dest =
            e.to.isCoPort === true
              ? coPortNodes[e.to.coCompositionElementName]
              : portNodes[e.to.compositionElementName];
          internalGraph.addEdge({
            type: "loop",
            mul: true,
            from: {
            ...
            .forEach((e1) => {
              internalGraph
                ...
                .forEach((e2) => {
                  ...
                  if (
                    _.isEmpty(
                      internalGraph.findUndirectedEdge({
                        from: {
                          node: e1.to.node,
                        },
                        to: {
        ...
        .forEach((internalEdge) => {
          ...
          graph
            .matchUndirectedEdges({
              from: {
                node: n1,
                coCompositionElementName:
                  internalEdge.from.node.ports.coCompositionElementName,
              },
            })
            .forEach((coEdge) => {
              graph
                .matchUndirectedEdges({
                  from: {
                    ...
                    compositionElementName:
                      internalEdge.to.node.ports.compositionElementName,
                  },
                })
                .forEach((edge) => {
