...
export default function nonMatchingCompositionCompilation(graph) {
  ...
  graph
    ...
    .filter(
      (x) =>
        madeOnlyOf(x.ports[0]) === "out" || madeOnlyOf(x.ports[0]) === "in",
    )
    .forEach((theNode) => {
      ...
      let compositionElementNameWithTheirIndex = graph
        ...
        .forEach((theEdge) => {
          if (theEdge.to.node === theNode) {
            throw new Error(
              "Error:Loop on a composition interaction " +
                theNode.content.operator,
            );
          }
        })
        .map((theEdge) => ({
      ...
      _(compositionElementNameWithTheirIndex)
        .forEach((el) => {
          if (isCompo) {
            code = code.concat(
              "<%=a0%>['" +
                el.compositionElementName +
                "'] = <%=a" +
                el.index +
                "%>;\n",
            );
            ports[el.index] = conjugateInterface(
              subInterface(theNode.ports[0], el.compositionElementName),
            );
          } else {
            code = code.concat(
              "<%=a" +
