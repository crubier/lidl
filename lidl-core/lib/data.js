...
var isValidData = function isValidData(theData) {
...
var computeData = function computeData(theData) {
    ...
    switch (theData.operator) {
      case 'union':
        if (_.every(theData.operand, "type", "DataComposite")) {
          return { 
            type: "DataComposite", 
            element: _.sortBy(_.unique(_.union.apply(null, _.map(theData.operand, "element")), "key"), "key") };} else 
      ...
      case 'intersection':
        if (_.every(theData.operand, "type", "DataComposite")) {
          return { 
            type: "DataComposite", 
            element: _.sortBy(_.unique(_.intersectionObjects.apply(null, _.map(theData.operand, "element")), "key"), "key") };} else 
...
var compareData = function compareData(data1, data2) {
