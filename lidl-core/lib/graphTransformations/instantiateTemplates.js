...
instantiateTemplates;var _lodash = require('lodash');var _lodash2 = _interopRequireDefault(_lodash);var _ports = require('../ports');function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}function instantiateTemplates(graph) {

  graph.
  matchNodes({ type: 'InteractionInstance' }).
  forEach(function (n) {n.shouldDoCodeGeneration = true;n.codeGeneration = false;}).
  commit();
  ...
  function (theResult, theNode) {
    ...
    forEach(function (port, index) {
      ...
      if (edge === undefined) {
        ...
        if ((0, _ports.portIsOnlyMadeOf)(port, 'in')) {
          // console.log("IN");
          ...
          edge = 
          graph.
          addEdge({ type: 'InteractionInstanceOperand', from: { node: newNode, index: 0, ports: 'out' }, to: { node: theNode, index: index, ports: port } });} else 
        if ((0, _ports.portIsOnlyMadeOf)(port, 'out')) {
          // console.log("OUT");
          ...
          edge = 
          graph.
          addEdge({ type: 'InteractionInstanceOperand', to: { node: newNode, index: 0, ports: 'in' }, from: { node: theNode, index: index, ports: port } });} else 
        {
    ...
    if (shouldGenerate) {
      // console.log(theArgs)
      theNode.codeGeneration = { 'js': _lodash2.default.template(theNode.content.content)(theArgs) };
