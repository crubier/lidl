...
describe("lidl graph compiler", function () {
  ...
  function test(file, commonHeader) {
    function printGraph(graph, name) {
      ...
      fs.writeFileSync(dotFile, graph.toDot(), { encoding: "utf8" });
      Bun.spawnSync(["dot", dotFile, "-o" + pdfFile, "-Tpdf"]);
    }
    ...
    fs.rmSync(path.join(file, "result"), { recursive: true, force: true });
    fs.rmSync(path.join(file, "dot"), { recursive: true, force: true });
    fs.rmSync(path.join(file, "pdf"), { recursive: true, force: true });
    ...
    describe("Compilation of file " + file, function () {
      ...
      graphCompiler.compile(parser.parse(code)[0], header, {
        ...
        getJsCode: function (graph, data) {
          fs.writeFileSync(
            path.join(file, "result", "generated.js"),
            data.source,
            {
              encoding: "utf8",
            },
