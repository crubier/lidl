...
function simplifyPathExpression(graph, p) {var reduced = (0, _lodash2.default)(p).groupBy('compositionNode.id') // We group path that go to the same target
  ...
  .forEach(function (x) {var allConds = (0, _lodash2.default)(x.condition).flatten().map(function (c) {return { cond: c, stringRep: c.node.id + " " + c.index };}).unique('stringRep').value();if (allConds.length == 0) {x.simplifiedCond = null;} else {if (allConds.length == 1) {// We do not have to create a new Synthetic node in this case
        x.simplifiedCond = x.condition[0][0];} else {(function () {var satProblem = (0, _lodash2.default)(x.condition).map(function (y) {return (0, _lodash2.default)(y).map(function (c) {return c.node.id + " " + c.index;}).value();}).value(); //           console.log("hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh");
          ...
          var newNode = graph.addNode({ type: 'InteractionInstance', content: { type: 'InteractionNative', content: "TBD" } });var allEdgesToConds = (0, _lodash2.default)(allConds).map(function (c, index) {return { cond: c.cond, stringRep: c.stringRep, edge: graph.addEdge({ type: 'InteractionInstanceOperand', from: { index: index + 1, node: newNode, ports: 'in' }, to: c.cond }) };}).value();var code = (0, _lodash2.default)(solution).map(function (s) {var solcode = "if( ";solcode = solcode + (0, _lodash2.default)(s).map(function (value, index) {return '( <%=a' + (index + 1) + '%>' + ' === ' + (value ? 'active' : 'inactive') + ' )';}).join(' && ');solcode = solcode + ") {\n  <%=a0%> = active;\n} else {\n  <%=a0%> = inactive;\n}";return solcode;}).join("\n"); // console.log(code);
          newNode.content.content = code;newNode.ports = ['out'].concat(_lodash2.default.map(allConds, 'in')); // console.log('ffffffffffffffffffffffffffffffffffffffffffffffffff');
          x.simplifiedCond = { index: 0, node: newNode, ports: 'out' };})();}}}).value(); // compositionNode
...
function matchingCompositionReduction(graph) {// // Mark all
  ...
  graph.reduceNodes({ type: 'InteractionInstance', unSuitableForCompositionReduction: false, content: { type: 'InteractionSimple', operatorType: 'Composition' } }, function (theResult, n1) {var matchingCompoNodes = simplifyPathExpression(graph, findAllMatchingCompositionNodesAndTheirAffectationCondition(graph, n1)); // console.log("       ");
    // console.log("       ");
    // console.log("       ");
    // console.log("       ");
    ...
    // console.log("       ");
    //
    (0, _lodash2.default)(matchingCompoNodes).forEach(function (x) {var n2 = x.compositionNode; // console.log("NULLLLSIMPLEONDDDDDDDD");
      ...
      if (_lodash2.default.isNull(x.simplifiedCond)) {// console.log("ok");
        ...
        graph.matchUndirectedEdges({ type: 'InteractionInstanceOperand', from: { node: n2 } }).reject(function (e2) {return _lodash2.default.isUndefined(e2.from.compositionElementName);}).forEach(function (e2) {// console.log(d2.id);
          ...
          // console.log(d2.node.id);
          graph.
          addEdge({ type: 'InteractionInstanceOperand', from: { node: n1, coCompositionElementName: e2.from.compositionElementName, isCoPort: true }, to: d2 });}).
        commit();
        ...
        n2.didCompositionReduction = true;} else 
      {
        ...
        // console.log("pb");
        graph.
        matchUndirectedEdges({ type: 'InteractionInstanceOperand', from: { node: n2 } }).
        ...
        forEach(function (e2) {
          ...
          graph.
          addEdge({ type: 'InteractionInstanceOperand', from: { node: intermedAffectation, index: 2 }, to: d2 });

          graph.
          addEdge({ type: 'InteractionInstanceOperand', from: { node: intermedAffectation, index: 0 }, to: x.simplifiedCond });}).
  ...
  graph.
  matchNodes({ type: 'InteractionInstance', didCompositionReduction: true }).
  forEach(function (n1) {
    // console.log("-----------------------------------------------");
    // console.log(n1.id);
    // gexport(graph,n1.id);
    ...
    var coPortsFrom = 
    graph.
    matchDirectedEdges({ type: 'InteractionInstanceOperand', from: { node: n1 } }).
    filter(function (x) {return x.from.isCoPort === true;}).
    map(function (x) {return { port: x.from, closed: false };}).
    value();

    var coPortsTo = 
    graph.
    matchDirectedEdges({ type: 'InteractionInstanceOperand', to: { node: n1 } }).
    filter(function (x) {return x.to.isCoPort === true;}).
    map(function (x) {return { port: x.to, closed: false };}).
    value();
    ...
    forEach(function (x) {coPortNodes[x.port.coCompositionElementName] = internalGraph.addNode(x);}).
    commit();
    ...
    forEach(function (x) {portNodes[x.port.compositionElementName] = internalGraph.addNode(x);}).
    commit();
    ...
    forEach(function (n1) {
      ...
      filter(function (x) {return x.port.isCoPort !== true;}).
    ...
    graph.
    matchUndirectedEdges({ type: 'InteractionInstanceOperand', from: { node: n1 }, to: { node: n1 } })
    // .forEach(e=>{
    ...
    function (theResult, theNode) {
      ...
      forEach(function (e1) {
        ...
        forEach(function (e2) {
          ...
          if (_lodash2.default.isEmpty(internalGraph.findUndirectedEdge({ from: { node: e1.to.node }, to: { node: e2.to.node } }))) {
            internalGraph.
    ...
    .filter(function (x) {return x.to.node.port.isCoPort !== true && x.from.node.port.isCoPort === true;}).
    forEach(function (internalEdge) {
      ...
      forEach(function (coEdge) {
        ...
        forEach(function (edge) {
          ...
          graph.
          addEdge({ type: 'InteractionInstanceOperand', from: coEdge.to, to: edge.to });}).
        commit();}).
      commit();}).
    commit();}).
  commit();
  ...
  graph.
  matchNodes({ type: 'InteractionInstance', didCompositionReduction: true, content: { type: 'InteractionSimple', operatorType: 'Composition' } }).
