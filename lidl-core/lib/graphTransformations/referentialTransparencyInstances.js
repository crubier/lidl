...
referentialTransparencyInstances;var _lodash = require('lodash');var _lodash2 = _interopRequireDefault(_lodash);function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} // During this phase we add the folowing decoration to nodes:
...
function referentialTransparencyInstances(graph) {// First we mark all nodes as not referentialTransparencySolved
  graph.matchNodes({ type: 'InteractionInstance', content: { type: 'InteractionSimple' } }).forEach(function (theNode) {
  ...
  function (theResult, theNode) {
    ...
    .filter(function (n) {return (// All chidren of similarNode are children of theNode
        graph.
        matchUndirectedEdges({ type: 'InteractionInstanceOperand', from: { node: n } }).
    ...
    forEach(function (ce) {
      graph.
      addEdge({ type: 'InteractionInstanceOperand', from: { node: newNode, index: ce.from.index, ports: ce.from.ports }, to: ce.to });}).
    commit();
    ...
    forEach(function (similarNode) {
      graph.
      matchUndirectedEdges({ type: 'InteractionInstanceOperand', from: { node: similarNode, index: 0 } }).
      forEach(function (edgeFromSimilarNode) {
        // console.log()
        graph.
        addEdge({ type: 'InteractionInstanceOperand', from: { node: newNode, index: edgeFromSimilarNode.from.index, ports: edgeFromSimilarNode.from.ports }, to: edgeFromSimilarNode.to });}).
      commit();
      graph.
      finish(similarNode);}).
    commit();
    ...
    graph.
    matchNodes({ type: 'InteractionInstance', content: { type: 'InteractionSimple' }, referentialTransparencySolved: false, referentialTransparencySolvable: false }).
    filter(function (n) {
      return graph.
      matchUndirectedEdges({ type: 'InteractionInstanceOperand', from: { node: n } }).
