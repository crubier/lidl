...
export default function linkArguments(graph, rootDefNode) {
  graph
    ...
    .forEach((edge) => {
      ...
      if (
        signatureOperandElementNode.content.name !==
        argumentInteractionInstanceNode.content.operator
      ) {
        throw new Error(
          "Internal error: arguments with matching indices have different names " +
            signatureOperandElementNode.content.name +
            " and " +
            argumentInteractionInstanceNode.content.operator,
        );
