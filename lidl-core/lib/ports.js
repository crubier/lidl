...
function portIsOnlyMadeOf(x, s) {if (_lodash2.default.isArray(x)) {var i = 0;var res = true;for (i = 0; i < x.length; i++) {res = res && portIsOnlyMadeOf(x[i], s);}return res;} else if (_lodash2.default.isString(x)) {if (x === s) {return true;} else {return false;}} else if (_lodash2.default.isUndefined(x)) {return false;} else {throw new Error("ports should be arrays of strings or strings");}} // ['in','out','out'] -> ['out','in','in']
function conjugatePort(x) {if (_lodash2.default.isArray(x)) {var i = 0;var res = [];for (i = 0; i < x.length; i++) {res[i] = conjugatePort(x[i]);}return res;} else if (_lodash2.default.isString(x)) {if (x === 'in') {return 'out';} else if (x === 'out') {return 'in';} else {throw new Error("ports should be in our out or arrays of in and out");}} else if (_lodash2.default.isUndefined(x)) {return undefined;} else {throw new Error("ports should be arrays of strings or strings");}}function portIsDefined(x) {if (_lodash2.default.isNull(x) || _lodash2.default.isUndefined(x)) {return false;} else {return true;};}