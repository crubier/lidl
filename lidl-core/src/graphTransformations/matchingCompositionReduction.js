...
// TODO use
  ...
  function visitTheAffectation(n,fromWhichSide) {
      ...
      n.temporarilyMarkedDuringCompositionTraversal = true;
          ...
          graph
          .matchUndirectedEdges({type: 'InteractionInstanceOperand',from:{node:n,index:0},to:{node:{type: 'InteractionInstance'}}})
          .map(e=>e.to)
          .value();
...
//           console.log("iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii");
          ...
          let newNode =
          graph
          .addNode({type:'InteractionInstance',content:{type:'InteractionNative',content:"TBD"}});

          let allEdgesToConds =
          _(allConds)
          .map((c,index)=>({cond:c.cond,stringRep:c.stringRep,edge:graph.addEdge({type:'InteractionInstanceOperand',from:{index:(index+1),node:newNode,ports:'in'},to:c.cond})}))
          .value();
...
// // Mark all
  graph
  .matchNodes({type: 'InteractionInstance',content: {type: 'InteractionSimple'}})
  ...
  graph
  .reduceNodes({type: 'InteractionInstance',unSuitableForCompositionReduction:false,content: {type: 'InteractionSimple',operatorType:'Composition'}},
  (theResult,n1)=>{
