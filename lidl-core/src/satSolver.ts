 ...
 * Solver main loop.
 */
export function satSolve(size: number, clauses: number[][]): boolean[] | null {
  // Create the state:
  var state = new State();
  ...
  for (var i = 1; i < state.vars.length; i++) {
    ...
    if (v.unit) {
      ...
      if (!satUnitPropagate(state, literal, null)) return null;
    }
  }
...
export function solvePath(nodes: any[], paths: any[][]): boolean[][] {
  var varNumber = nodes.length;
  var clauses = _(paths)
    .map((thePath) =>
      _(thePath)
        .map((theElement) => -1 * (_.indexOf(nodes, theElement) + 1))
        .value(),
    )
    .value();
