...
describe("operator", function () {
  describe("previous", function () {
    it("should detect previous", function () {
      expect(operator.parse("get $ from previous and set $ for next")).toEqual(
        "Previous",
      );
    });
  });
  ...
  describe("composition", function () {
    ...
    it("should detect composition with mixed commas ", function () {
      expect(operator.parse("{bobie:$,joe:$,lol:$wow:$}")).toEqual(
        "Composition",
      );
    });
  ...
  describe("function application", function () {
    it("should detect basic function application", function () {
      expect(operator.parse("apply$to$and get$")).toEqual(
        "FunctionApplication",
      );
    });
  });
  ...
  describe("with parsed interaction", function () {
    it("should work on a parsed interaction", function () {
      expect(
        operator.parse(
          parser.parse("(wow(5)lolie(6))", { startRule: "interaction" })
            .operator,
        ),
      ).toEqual("Custom");
    });
