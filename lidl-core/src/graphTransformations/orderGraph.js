...
export default function orderGraph(graph) {
  ...
  graph.reduceNodes(
    { markedDuringGraphOrdering: false },
    (theResult, theNode) => {
  ...
  function visit(n, stack) {
    if (n.temporarilyMarkedDuringGraphOrdering === true) {
      ...
      throw new Error(
        "the interaction DAG contains cycles: " +
          _(stack).concat([n]).map("id").join(" -> "),
      );
    } else {
          ...
          .matchNodes(
            (m) =>
              graph
                ...
                .filter(
                  (edge) =>
                    madeOnlyOf(edge.from.ports) === "out" &&
                    madeOnlyOf(edge.to.ports) === "in",
                )
                .size() > 0,
          )
  ...
  _(orderingList).reduce((prev, current, index) => {
