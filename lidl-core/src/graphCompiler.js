...
export function compile(ast, header, callbacks) {
  ...
  function callCallback(element, data) {
    ...
    if (_.isUndefined(callbacks[element])) {
      ...
      if (_.isBoolean(ret)) {
        ...
        throw new Error(
          "Compilation stage callbacks should return a boolean (true to continue compilation, false to stop it), the callback for " +
            element +
            " returned " +
            ret,
        );
  ...
  var newCallbacks = _.assign(_.clone(callbacks), {
    instantiateInterfaces: (graph, data) => {
      callCallback(
        "getExpandedLidlCode",
        getExpandedLidl(graph, rootDefinitionNode),
      );
...
export function graphTransformationPipeline(
  graph,
  rootDefinitionNode,
  callbacks,
) {
  ...
  try {
    ...
    if (false === callCallback("addInterfaceInformationToInteractions"))
      return graph;
    ...
    if (false === callCallback("referentialTransparencyInstances"))
      return graph;
