...
var removeFormattingInfo = function (x) {
  return interactions.removeFormattingInfo(removeMeta(x));
};
...
function parse(x, options) {
  var opts;
  if (options === undefined) opts = { startRule: "start" };
  else opts = options;
...
describe("interactions", function () {
  describe("shallow list of elements", function () {
    it("simple2", function () {
      ...
      expect(
        interactions.fromShallowListOfElements(
          interactions.toShallowListOfElements(k),
        ),
      ).toEqual(k);
    });
  });
  ...
  describe("to shallow list of elements", function () {
    it("too simple", function () {
      expect(
        interactions.toShallowListOfElements({
          type: "InteractionSimple",
          operator: "oksss",
          formating: "oksss",
          operand: [],
  ...
  describe("to list of elements", function () {
    ...
    it("simple2", function () {
      ...
      ).toEqual(["oo wow os", ["bob"], "ollslk", ["lol"], "bobie joe"]);
    });
  });
  ...
  describe("list of interactions", function () {
    it("simple", function () {
      expect(
        interactions.listOfInteractions(
          parse("(a)", {
  ...
  describe("is base interaction", function () {
    it("simple", function () {
      expect(
        interactions.isBaseInteraction(
          parse("(variable (q))", {
            startRule: "interaction",
          }),
        ),
  ...
  describe("is made of base interactions", function () {
    it("case 1", function () {
      expect(
        interactions.isOnlyMadeOfBaseInteractions(
          parse(
            "(get (variable x) from previous and set (variable x) for next)",
            {
              startRule: "interaction",
            },
          ),
    ...
    it("case 4 with all base interactions", function () {
      expect(
        interactions.isOnlyMadeOfBaseInteractions(
          parse(
            "({a:(previous(variable (variable 5)))b:((variable d).xys)c:((variable 2)in(variable 5)(variable 2)=(variable (variable 5)))})",
            {
              startRule: "interaction",
            },
          ),
      ...
      expect(
        removeFormattingInfo(
          interactions.instantiate(
            ...
            parse(
              "interaction (a(x:Number in)):Number out is (bob(x)test(x))",
              {
                startRule: "interactionDefinition",
              },
            ),
      ...
      expect(
        removeFormattingInfo(
          interactions.instantiate(
            ...
            parse(
              "interaction (a(x:Number in)):Number out is (bob(x)test(b))",
              {
                startRule: "interactionDefinition",
              },
            ),
  ...
  describe("find matching definition", function () {
    it("should work on a simple case with a definition on the same level", function () {
      var res = parse(
        "interaction (a(x:Number in)):Number out with interaction (b):Number out is (c) is (bob(x)test(b))",
        {
          startRule: "interactionDefinition",
        },
      );
    ...
    it("should work on a simple case with a definition on the parent", function () {
      var res = parse(
        "interaction (k):Number in with interaction (b):Number out is (d) interaction (a(x:Number in)):Number out with interaction (b):Number out is (c) is (bob(x)test(b)) is (b)",
      );
      var b = parse("(b)", {
        startRule: "interaction",
      });
      ...
      var foundDefb = interactions.findMatchingDefinition(
        b,
        res[0].definitions[1],
      );
      delete foundDefb.parent;
    ...
    it("should work on a simple case where the interaction is an argument", function () {
      ...
      var foundDefx = interactions.findMatchingDefinition(
        x,
        res[0].definitions[1],
      );
    ...
    it("should work on a simple case where the interaction is an argument of a parent", function () {
      var res = parse(
        "interaction (k(x:Number in)):Number in with interaction (b):Number out is (d) interaction (a):Number out with interaction (b):Number out is (c) is (bob(x)test(b)) is (b)",
      );
      var x = parse("(x)", {
        startRule: "interaction",
      });
  ...
  describe("list non base interactions", function () {
    it("should work on a simple case", function () {
      var list = interactions.listNonBaseInteractions(
        removeMeta(
          parse("({b:(4),y:(cos(sin(previous(x))))})", {
            startRule: "interaction",
          }),
        ),
      );
  ...
  describe("expand", function () {
    it("should work on a simple case", function () {
      ...
      expect(removeFormattingInfo(interaction)).toEqual(
        removeFormattingInfo(
          parse("(x)", {
            startRule: "interaction",
          }),
    ...
    it("should work on a simple case with a closure", function () {
      var interaction = interactions.expand(
        parse(
          "interaction (joe(x:Number in)):Number in with interaction (a):Number in is (x) is (a)",
        )[0],
      ).interaction;
      expect(removeFormattingInfo(interaction)).toEqual(
        removeFormattingInfo(
    ...
    it("should error on a erroneous case", function () {
      expect(function () {
        interactions.expand(
          parse(
            "interaction (joe(x:Number in)):Number in with interaction (a):Number in is (wjxijwx) is (a)",
          )[0],
        );
    ...
    it("should work on a simple case with base interactions", function () {
      var interaction = interactions.expand(
        parse(
          "interaction (a(x:Number in)(y:Number out)):Number in is ({x:(x),y:(y)})",
        )[0],
      ).interaction;
