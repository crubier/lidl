...
export function serialize(object: any, indentation?: number): string {
  if (_.isUndefined(object)) return "?";
  var indent;
  if (!_.isFinite(indentation)) {
    indent = 0;
  } else {
    indent = indentation;
  }
  switch (object.type) {
    ...
    case "DataFunction":
      return (
        "{" +
        serialize(object.domain) +
        " -> " +
        serialize(object.codomain) +
        "}"
      );
    case "DataArray":
...
var indentString = "  ";
// Interaction
function serializeInteractionSimple(object: any, indentation: number): string {
...
export function serializeInteractionSimpleRow(object: any): string {
...
function serializeInteractionSimpleNeat(object: any, indentation: number): string {
  ...
  if (object.operand.length === 0) {
    ...
    res =
      res +
      (list[0].length === 0 ? "" : indentString + list[0] + "\n" + baseIndent);
    for (var i = 1; i < list.length; i++) {
      res =
        res +
        indentString +
        serialize(object.operand[i - 1], indentation + 1) +
        "\n" +
        baseIndent;
      res =
        res +
...
function serializeInteractionSignatureSimpleRow(object: any, indent: number): string {
  if (object.operand.length === 0) {
    return "(" + object.operator + ")";
  } else {
    var list = object.operator.split("$");
