...
export function listOfInteractions(theInteraction: any): any[] {
...
export function expand(interactionDefinition: any): any {
  ...
  do {
    interactionsToExpandAlongWithTheirMatchingDefinition = _.filter(
      _.map(listNonBaseInteractions(interaction), function (x) {
        return {
          interaction: x,
          definition: findMatchingDefinition(
            x,
            interactionDefinitionWithChildrenExpanded,
          ),
        };
      }),
      function (y) {
        return (
          !isDefinitionOfAnArgument(y.definition) &&
          y.interaction.type === "InteractionSimple"
        );
      },
    );
...
export function instantiate(interaction: any, interactionDefinition: any): any {
    ...
    case "InteractionSimple": {
      ...
      if (isDefinitionOfAnArgument(interactionDefinition)) {
        ...
        if (interactionMatchesDefinition(interaction, interactionDefinition)) {
          return _.reduce(
            _.zip(
              interactionDefinition.signature.operand,
              instantiatedOperands,
            ),
            function (accumulator, value, key, collection) {
              return substituteInInteraction(
                accumulator,
                {
...
export function findMatchingDefinition(interaction: any, interactionDefinition: any): any {
  // First case : No definition
  if (interactionDefinition === undefined) {
    throw new Error(
      "could not find definition matching interaction " +
  ...
  for (var i = 0; i < interactionDefinition.definitions.length; i++) {
    if (
      interactionMatchesDefinition(
        interaction,
        interactionDefinition.definitions[i],
      )
    ) {
  ...
  throw new Error(
    "cannot find definition of interaction " + interaction.operator,
  );
}

// Checks if an interaction definition states that an interaction is an argument
function isDefinitionOfAnArgument(definition: any): boolean {
...
export function interactionMatchesDefinition(interaction: any, interactiondefinition: any): boolean {
  ...
  if (a.operator > b.operator) {
    ...
    if (a.operator < b.operator) {
      ...
      if (
        ...
        if (
          (a.operand === undefined || a.operand === null) &&
          (b.operand === undefined || b.operand === null)
        ) {
          return 0;
        } else {
          return -1;
...
export function substituteInInteraction(theInteraction: any, target: any, substitute: any): any {
  switch (theInteraction.type) {
    case "InteractionNative": {
...
export function isOnlyMadeOfBaseInteractions(interaction: any): boolean {
...
export function listNonBaseInteractions(interaction: any): any[] {
  if (interaction.type === "InteractionSimple") {
    return (isBaseInteraction(interaction) ? [] : [interaction]).concat(
      _.flatten(_.map(interaction.operand, listNonBaseInteractions)),
    );
  } else {
    return [];
  }
...
export function isBaseInteraction(interaction: any): boolean {
...
export function toListOfElements(interaction: any): any[] {
  var i = 0;
...
function concat(x: any, y: any): [any, any] {
  return [x, y];
}

export function toShallowListOfElements(interaction: any): any[] {
  return _.dropRight(
    _.flatten(
      _.zipWith(
        interaction.formating.split("$").map(normalize),
        interaction.operand,
        concat,
      ),
    ),
...
export function fromShallowListOfElements(lst: any[]): any {
  var formating = lst
    .map(function (x) {
      if (_.isString(x)) {
        return x;
...
export function removeFormattingInfo(interaction: any): any {
  return {
...
export function getFormattingInfo(interaction: any): string {
