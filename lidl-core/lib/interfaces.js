...
function mergeInterface(x, y) {// console.log("merge");
  ...
  if (isCompatible(x, conjugateInterface(y))) {if (isComposite(x)) {if (isComposite(y)) {return { type: 'InterfaceComposite', element: (0, _lodash2.default)(x.element).concat(y.element).
      ...
      if (isUndefined(y)) {
        return x;} else 
      {
        throw new Error('Trying to merge with something which is not an interface: ' + JSON.stringify(y));}} else 
    ...
    {
      throw new Error('Trying to merge with something which is not an interface: ' + JSON.stringify(x));}} else 
