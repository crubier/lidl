...
import { mergeInterface, isAtomic, conjugateInterface } from "../interfaces";
  ...
  graph.reduceNodes(
    ...
    (theResult, theNode) => {
      ...
      graph
        ...
        .forEach((x) => {
          source.ports[1] = mergeInterface(
            source.ports[1],
            conjugateInterface(x.to.node.ports[x.to.index]),
          );
          graph.addEdge({
            type: "InteractionInstanceOperand",
            from: {
      ...
      if (
        !isAtomic(source.ports[1]) ||
        source.ports[1].data.type !== "DataFunction"
      )
        throw new Error(
          "Function application should receive functions as first argument, received the interface " +
            serialize(source.ports[1]),
        );
