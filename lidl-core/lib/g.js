"use strict";var _createClass = (function () {function defineProperties(target, props) {for (var i = 0; i < props.length; i++) {var descriptor = props[i];descriptor.enumerable = descriptor.enumerable || false;descriptor.configurable = true;if ("value" in descriptor) descriptor.writable = true;Object.defineProperty(target, descriptor.key, descriptor);}}return function (Constructor, protoProps, staticProps) {if (protoProps) defineProperties(Constructor.prototype, protoProps);if (staticProps) defineProperties(Constructor, staticProps);return Constructor;};})();var _lodash = require('lodash');var _lodash2 = _interopRequireDefault(_lodash);var _serializer = require('./serializer');function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}function _classCallCheck(instance, Constructor) {if (!(instance instanceof Constructor)) {throw new TypeError("Cannot call a class as a function");}}var 
  ...
  _createClass(Graph, [{ key: 'addNode', value: function addNode(
    ...
    // Match all unfinished nodes that comply to a pattern
      ...
      if (pattern === undefined) {
        return (0, _lodash2.default)(this.nodes).filter(this.nodeIsNotFinished.bind(this));} else 
      if (pattern.type !== undefined) {
        return (0, _lodash2.default)(this.nodeTypeIndex[pattern.type]).filter(this.nodeIsNotFinished.bind(this)).filter(pattern);} else 
      {
    ...
    // Match all unfinished edges that comply to a pattern
      ...
      if (pattern === undefined) {// No pattern, we return all edges of the graph
        return (0, _lodash2.default)(this.edges).filter(this.edgeIsNotFinished.bind(this));} else 
    ...
    // Find an edge that comply to a pattern
      ...
      if (pattern === undefined) {// No pattern, we return all edges of the graph
        return (0, _lodash2.default)(this.edges).find(this.edgeIsNotFinished.bind(this));} else 
      if (pattern.type !== undefined) {// We have a pattern and it specifies a type of edge
        if (pattern.from !== undefined && pattern.to !== undefined && pattern.from.node !== undefined && pattern.to.node !== undefined) {
          if (pattern.from.node.outgoingEdgeTypeIndex !== undefined) {// The node is an actual graph node with indexes and stuff
            return (0, _lodash2.default)(pattern.from.node.outgoingEdgeTypeIndex[pattern.type]).
    ...
    // otherwise the call will loop forever
      ...
      return (0, _lodash2.default)(result);} }, { key: 'reduceDirectedEdges', value: function reduceDirectedEdges(
    ...
    pattern, iteratee, accumulator, thisArg) {
      ...
      return (0, _lodash2.default)(result);} }, { key: 'reduceUndirectedEdges', value: function reduceUndirectedEdges(
    ...
    e) {
      return !this.edgeIsFinished(e);} }, { key: 'toDotWithParamaters', value: function toDotWithParamaters(
